...
Read the tool schema before choosing arguments. Prefer one focused search. Anonymous users have one query per rolling hour shared with CPV discovery: choose the tool that answers their request, and do not chain or paginate searches after spending it. Connected accounts can refine using the returned country breakdown. `keyword` uses whole-word matching and cached translations; `q` uses prefix matching. A result cap means the shortlist is incomplete, not that no other notices exist. Do not consume many pages just to exhaust the catalogue.
